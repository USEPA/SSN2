test_that("ssn manipulation functions work", {
  ...
  file_names <- c("binaryID.db", "CapeHorn.gpkg", "edges.gpkg", "netID1.dat",
  ...
  ssn_create_distmat(mf04p_new, predpts = c("pred1km"),
  ...
  pred1km_files <- c("dist.net1.RData", "dist.net2.RData", "dist.net1.a.RData",
  ...
  ssn_subset(mf04p_new, path = subset_path,
  ...
  mf04p_new_split <- ssn_split_predpts(mf04p, "pred1km", size_predpts = 100,
  ...
  mf04p_new_split <- ssn_split_predpts(mf04p, "pred1km", by = "net.fac",
                                       keep = FALSE, overwrite = TRUE)
  expect_identical(names(mf04p_new_split$preds), c("CapeHorn", "pred1km-net.fac-1", "pred1km-net.fac-2"))
  mf04p_new_split <- ssn_split_predpts(mf04p, "pred1km", subset = ratio > 0.5,
