...
1.  `SSN` depends on the `rgdal` [@bivand2021rgdal], `rgeos` [@bivand2020rgeos], and `maptools` [@bivand2021maptools] **R** packages, which were retired in October, 2023. Their functionality has been replaced and modernized by the `sf` package [@pebesma2018sf]. `SSN2` depends on `sf` instead of `rgdal`, `rgeos`, and `maptools`, reflecting this broader change regarding handling spatial data in **R**.
...
2.  There are features added to `SSN2` that would have been difficult to implement in `SSN` without a massive restructuring of `SSN`'s foundation, so a new package was created. For example, the `SSN` objects in `SSN2` are S3 objects but the `SSN` objects in `SSN` were S4 objects. Additionally, many functions were rewritten and/or repurposed in `SSN2` to use generic functions (e.g., block prediction in `SSN2` is performed using `predict()` while in `SSN` it was performed using `BlockPredict()`). Moreover, `SSN2` leverages many of the tools in the `spmodel` **R** package for spatial statistical modeling [@dumelle2023spmodel].
...
network models in the `SSN2` package. We recommend pre-processing data directly in **R** via the new `SSNbler` **R** package, which you can learn more about by visiting the `SSNbler` GitHub page at [this link](https://github.com/pet221/SSNbler). Legacy options for pre-processing data include the Spatial Tools for the Analysis of River Systems
(STARS) toolset for ArcGIS Desktop versions 9.3x-10.8x
[@peterson2014stars] and the `openSTARS` R package
[@kattwinkel2020openSTARS], which leveraged GRASS GIS. The pre-processing yields a non-proprietary `.ssn` folder (i.e., directory) which contains all
