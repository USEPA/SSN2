...
We create a path to the directory and read in the data using `ssn_import()`, naming the `SSN` object `mf04p`:
```{r}
temp_path <- system.file("lsndata/MiddleFork04.ssn", package = "SSN2")
mf04p <- ssn_import(
  path = temp_path,
...
When creating `.ssn` objects using `STARS` or `openSTARS`, users will only have to call `ssn_import()`, specifying the file path where the `.ssn` object is stored.
