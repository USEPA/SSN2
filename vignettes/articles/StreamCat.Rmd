...
author: "Michael Dumelle, Erin Peterson, Marc Weber, and Ryan Hill"
...
The `bugs` data is an SSN object that contains macroinvertebrate data in the Lower Snake Basin. It can be downloaded via GitHub at [this link](https://github.com/USEPA/SSN2/tree/develop/vignettes/articles). We read in the bugs by running:
```{r}
# library(here)
...
We visualize the flowlines, observed macroinvertebrate richness, and prediction sites by running:
```{r}
ggplot() +
...
If you only have access to x- and y-coordinates (instead of COMIDs), you can use `sc_get_comid()` to find the NHD feature that contains a coordinate by running:
```{r}
...
Alternatives to StreamCat? Primarily, other data sources outside CONUS? e.g., Canada, Australia, Europe, New Zealand, Africa, etc.
